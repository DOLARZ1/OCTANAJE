...
(function () {
  ...
  function nut() {
    ...
    if (!Array.isArray(s.nutrition.drinkCats)) s.nutrition.drinkCats = [];
    return s.nutrition;
  }
  ...
  // ---------------- Categorías (agregar nuevas / renombrar cualquiera) ----------------
  // Cada alimento guarda una "clave" de categoría fija e interna (p.ej. "Bebidas"),
  // que nunca cambia aunque la renombres — así la lógica de mililitros/gramos y el
  // filtrado siguen funcionando sin importar el nombre visible. catNames guarda solo
  ...
  function addCategory(name) {
    ...
    if (!clean) return { ok: false, msg: "Escribe un nombre para la categoría" };
    const dup = allCatKeys().some((k) => catLabel(k).toLowerCase() === clean.toLowerCase());
    if (dup) return { ok: false, msg: "Ya existe una categoría con ese nombre" };
  ...
  function renameCategory(key, newLabel) {
    ...
    if (!clean) return { ok: false, msg: "Escribe un nombre válido" };
    const dup = allCatKeys().some((k) => k !== key && catLabel(k).toLowerCase() === clean.toLowerCase());
    if (dup) return { ok: false, msg: "Ya existe otra categoría con ese nombre" };
    if (clean === key) { delete catNames()[key]; } else { catNames()[key] = clean; }
    Store.commit();
    return { ok: true };
  }
  function removeCategory(key) {
    const inUse = allFoods().some((f) => f.cat === key);
    if (inUse) return { ok: false, msg: "No se puede eliminar: hay alimentos usando esta categoría" };
    const arr = customCats(); const i = arr.indexOf(key);
    if (i < 0) return { ok: false, msg: "Esa categoría no se puede eliminar" };
    arr.splice(i, 1);
    delete catNames()[key];
    Store.commit();
    return { ok: true };
  ...
  function addEntry(food, grams, dateKey) {
    const f = grams / 100;
    const entry = {
  ...
  function isDrinkFood(food) {
    ...
    const cat = (food && typeof food === "object") ? food.cat : food;
    return cat === DRINK_CAT || nut().drinkCats.includes(cat);
  }
  ...
  function toggleDrinkCategory(catKey) {
    ...
    if (i >= 0) arr.splice(i, 1); else arr.push(catKey);
    Store.commit();
  }
  ...
  let browseCat = "Todos";
  let browseQuery = "";
  let browseDate = null; // fecha destino del registro; null = hoy

  function openBrowser(dateKey) {
    browseDate = dateKey || null;
    const isBackdate = !!browseDate && browseDate !== today();
    ...
    chips.appendChild(el("button", { class: "chip accent", style: "cursor:pointer", html: "🏷️ Categorías", onclick: () => openCategoryManager() }));

    const banner = isBackdate ? el("div", { class: "insight warn", style: "margin-bottom:10px" }, [
      el("span", { class: "ico", text: "🕐" }),
      el("div", { class: "txt", html: "Estás registrando para <b>" + dayLabel(browseDate) + "</b>, no para hoy." })
    ]) : null;

    const body = el("div", {}, [banner, search, chips, listWrap, addBtn]);
    UI.openModal(isBackdate ? "🍽️ Alimentos · " + dayLabel(browseDate) : "🍽️ Alimentos", body);
    renderList();
  }
  ...
  function foodRow(f) {
    ...
    return el("div", { class: "item", style: "padding:10px 12px;cursor:pointer", onclick: () => openPortion(f, browseDate) }, [
      el("button", {
        ...
        onclick: (ev) => { ev.stopPropagation(); toggleFavorite(f); openBrowser(browseDate); },
        html: fav ? "⭐" : "☆"
      }),
      ...
      f.custom ? el("button", { class: "icon-btn", title: "Eliminar de la lista", onclick: (ev) => {
        ...
        UI.confirmBox("Eliminar alimento", "¿Eliminar \"" + f.name + "\" de tu lista? (los registros ya guardados en tu historial no se borran)", () => {
          removeCustomFood(f); Audio.play("delete"); toast({ icon: "🗑️", msg: "Alimento eliminado de la lista" }); openBrowser(browseDate);
        }, "Eliminar");
      }, html: "🗑️" }) : null,
      el("button", { class: "icon-btn", html: "＋", title: "Agregar", onclick: (ev) => { ev.stopPropagation(); openPortion(f, browseDate); } })
    ]);
  }
  ...
  function openBackdatePicker() {
    ...
    const body = el("div", {}, [
      el("p", { class: "text-dim fs-13", style: "margin-bottom:10px", text: "Elige el día que quieres registrar (por ejemplo si se te olvidó anotar ayer o antier):" }),
      el("div", { class: "field" }, [el("label", { text: "Fecha" }), dateI]),
      el("button", { class: "btn primary block mt-8", html: "🍽️ Elegir alimentos de ese día", onclick: () => {
        if (!dateI.value) { Audio.play("error"); toast({ icon: "⚠️", msg: "Elige una fecha" }); return; }
        UI.closeModal();
        openBrowser(dateI.value);
      } })
    ]);
  ...
  function openCategoryManager() {
    ...
    const addBtn = el("button", {
      class: "btn primary block mt-8", html: "＋ Agregar categoría nueva",
      onclick: () => {
    ...
    UI.openModal("🏷️ Categorías de alimentos", body);
  }
  ...
  function openCustomFoodForm(existing) {
    ...
    function unitTag() { return isDrinkCategory(catI.value) ? "ml" : "g"; }
  ...
  function openPortion(food, dateKey) {
    ...
    const switchLabel = mode === "portion" ? "⚖️ Usar " + unitLabel(food) + " en vez de porciones" : "🍽️ Usar porciones en vez de " + unitLabel(food);
    const switchLink = hasPortion ? el("button", {
      ...
      onclick: (ev) => {
        ...
        switchLink.innerHTML = mode === "portion" ? "⚖️ Usar " + unitLabel(food) + " en vez de porciones" : "🍽️ Usar porciones en vez de " + unitLabel(food);
        paintFields(); paint();
      }
    }) : null;
    ...
    const dateBanner = isBackdate ? el("div", { class: "insight warn", style: "margin-bottom:14px" }, [
      el("span", { class: "ico", text: "🕐" }),
      el("div", { class: "txt", html: "Se registrará en <b>" + dayLabel(targetDate) + "</b>, no en hoy." })
    ]) : null;
    ...
    UI.openModal((isBackdate ? "Agregar (" + dayLabel(targetDate) + ") · " : "Agregar ") + food.name, body);
    paint();
  }
  ...
  function openDayDetail(dateKey) {
    ...
    body.appendChild(el("button", { class: "btn primary block mt-16", html: "＋ Agregar alimento a este día", onclick: () => { UI.closeModal(); openBrowser(dateKey); } }));
    body.appendChild(el("button", { class: "btn block mt-8", html: "📄 Descargar PDF de este día", onclick: () => { UI.closeModal(); exportPDF("day", dateKey); } }));
    ...
    if (!list.length) {
      ...
      dayKeys.forEach((dk) => {
        ...
        items.forEach((x) => {
          rows += "<tr>" +
            "<td>" + esc(x.name) + "</td>" +
            "<td style='text-align:center'>" + x.grams + " " + unitWord(x) + "</td>" +
  ...
  function render(container) {
    ...
    container.appendChild(el("div", { class: "view-head" }, [
      ...
      el("div", { class: "flex gap-8", style: "flex-wrap:wrap" }, [
        ...
        el("button", { class: "btn", onclick: openBackdatePicker, html: "🕐 Registrar día anterior" }),
        el("button", { class: "btn", onclick: () => openCategoryManager(), html: "🏷️ Categorías" }),
